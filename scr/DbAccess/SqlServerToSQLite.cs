    ...
    public class SqlServerToSQLite
    {
            ...
            bool createTriggers, bool createViews)
        {
            ...
            WaitCallback wc = new WaitCallback(delegate(object state)
            {
                ...
                {
                    _isActive = true;
                    ConvertSQLiteFileToSqlServerDatabase(sqlServerConnString, sqlitePath, password, handler, selectionHandler, viewFailureHandler, createTriggers, createViews);
                    _isActive = false;
        ...
        private static void ConvertSQLiteFileToSqlServerDatabase(
            string sqlConnString, string sqlitePath, string password, SqlConversionHandler handler,
            SqlTableSelectionHandler selectionHandler,
            FailedViewDefinitionHandler viewFailureHandler,
            bool createTriggers, bool createViews)
        {
            ...
            // Copy all rows from SQL Server tables to the newly created SQLite database
            CopySqLiteDBRowsToSqlServer(sqlConnString, sqlitePath, ds.Tables, password, handler);
        ...
        private static void CopySqLiteDBRowsToSqlServer(
            string sqlConnString, string sqlitePath, List<TableSchema> schema,
            string password, SqlConversionHandler handler)
        {
            CheckCancelled();
        ...
        private static object CastValueForColumn(object val, ColumnSchema columnSchema)
        {
            ...
            switch (dt)
            {
                ...
                case DbType.DateTime:
                    ...
                    if (tmp < new DateTime(1753,1,1)) return new DateTime(1753,1,1);
                    break;
